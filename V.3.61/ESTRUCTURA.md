...
## Explora Arquetipos (inspiración)

| Archivo | Rol |
|---------|-----|
| `ArchetypesCreator.tsx` | Contenedor principal: tres columnas Quién/Qué/Cómo, flujo Tinder (fase 1) → ajustes (fase 2), paleta general combinada. |
| `ArchetypeAxesColumn.tsx` | Columna derecha fase 2: ejes arquetipo (sliders, colores), reordenación drag, modal custom. |
| `archetypeAxesConfig.ts` | Configuración de ejes, órdenes por columna, `getFallbackAxisOrder`, `getEffectiveCharacterId`/`getEffectiveSilhouetteId`/`getEffectiveEstiloId`. |
| `archetypePaletteUtils.ts` | Mezcla de paletas (`blendColorsVibrant`, `combineColumnPalettes`), modos de combinación, `NEUTRAL_PALETTE`. |
| `ColumnSummaryModal.tsx` | Modal resumen antes de editar: tarjeta, ejes configurados, paleta vinculada. |
| `CustomAxisModal.tsx` | Modal para personalizar ejes (Identidad, Esencia, etc.). |
| `TinderCardPreview.tsx` | Vista previa de tarjeta con componentes (fondo, criatura/silueta/estilo, atmósfera). |
| `QuienTinderCards/`, `QuienSilhouettes/`, `ComoEstilos/` | Personajes, siluetas y estilos por columna. |

**Intercambio Quién↔Qué**: `getContentColumnKey` intercambia botones: "Quién" usa flow `que` (siluetas), "Qué" usa flow `quien` (criaturas). Para ejes: `columnKey 'que'` = UI Quién (Herramientas), `columnKey 'quien'` = UI Qué (Esencia). Ver `docs/ARQUETIPOS.md` para detalles.
...
- **Explora Arquetipos (Quién/Qué/Cómo):** `ArchetypesCreator.tsx`, `archetypeAxesConfig.ts`, `archetypePaletteUtils.ts`. Ver `docs/ARQUETIPOS.md`.
