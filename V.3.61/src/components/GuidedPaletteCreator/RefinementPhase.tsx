...
interface RefinementPhaseProps {
  ...
  /** Sliders de "Ajusta la paleta" (persistidos al cambiar de sección). */
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-0 flex-1 overflow-hidden">
        ...
        <div className="flex flex-col gap-4 min-h-0 overflow-y-auto">
          ...
          <div className={REFINEMENT_SLIDERS_BLOCK_MARGIN}>
            ...
            sliderTone={refinementGeneralSliders.tone}
            setSliderTone={(v) => setRefinementGeneralSliders((prev) => ({ ...prev, tone: v }))}
            sliderSat={refinementGeneralSliders.sat}
            setSliderSat={(v) => setRefinementGeneralSliders((prev) => ({ ...prev, sat: v }))}
            sliderLight={refinementGeneralSliders.light}
            setSliderLight={(v) => setRefinementGeneralSliders((prev) => ({ ...prev, light: v }))}
            />
          </div>
        </div>
