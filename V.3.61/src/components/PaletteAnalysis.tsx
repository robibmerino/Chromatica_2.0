...
import { hexToHsl, hslToHex, getContrastRatioHex } from '../utils/colorUtils';
...
export const PaletteAnalysis: React.FC<PaletteAnalysisProps> = ({ colors, onApplyFix }) => {
  ...
  const issues = useMemo(() => {
    ...
    for (let i = 0; i < analysisColors.length - 1; i++) {
      const contrast = getContrastRatioHex(analysisColors[i], analysisColors[i + 1]);
      if (contrast < 1.5) {
    ...
    const textContrast = getContrastRatioHex(analysisColors[darkestIdx], analysisColors[lightestIdx]);
    if (textContrast < 4.5) {
      detectedIssues.push({
  ...
  const originalScore = useMemo(() => {
    ...
    for (let i = 0; i < colors.length - 1; i++) {
      const contrast = getContrastRatioHex(colors[i], colors[i + 1]);
    ...
    const textContrast = getContrastRatioHex(colors[darkestIdx], colors[lightestIdx]);
