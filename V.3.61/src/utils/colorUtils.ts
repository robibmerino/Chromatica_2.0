...
/** Oscurece un color hex restando amount a cada canal RGB (0–255). */
...
/** Aclara un color hex sumando amount a cada canal RGB (0–255). */
...
/** Rota el matiz de un color hex en la rueda cromática. degrees en [-360, 360]. Si s≈0 (gris) no cambia. */
...
/**
 * Calcula la rotación de matiz para ejes arquetipo.
 * Usado por fondos y componentes que rotan su paleta según el slider y la personalización de extremos.
 *
 * @param colorLeft - Color actual del extremo izquierdo
 * @param colorRight - Color actual del extremo derecho
 * @param defaultColorLeft - Color predeterminado del extremo izquierdo (ancla; si no se pasa, se usa colorLeft)
 * @param sliderValue - 0-100
 * @returns Grados a rotar en la rueda cromática
 */
export function computeAxisHueRotation(
  colorLeft: string,
  colorRight: string,
  ...
  const hLeft = hexToHsl(colorLeft).h;
  const hRight = hexToHsl(colorRight).h;
  const hDefaultLeft = hexToHsl(defaultColorLeft).h;
  return (1 - t) * (hLeft - hDefaultLeft) + t * (hRight - hDefaultLeft);
}

/**
 * Rota todos los colores de una paleta por los grados indicados.
 * Útil para componentes que aplican la rotación del eje a una paleta base.
 */
export function rotatePaletteHues<T extends Record<string, string>>(
  palette: T,
  degrees: number
): T {
  const rotate = (hex: string) => rotateHue(hex, degrees);
  return Object.fromEntries(
    Object.entries(palette).map(([k, v]) => [k, rotate(v)])
  ) as T;
}
...
/** Luminancia relativa desde hex (para contrastes WCAG). */
...
/** Ratio de contraste entre dos colores hex (para contrastes WCAG). */
...
/**
 * Fondo con contraste para un elemento de color hex.
 * Si el color es claro (L > 55) → fondo negro con transparencia.
 * Si el color es oscuro → fondo blanco con transparencia.
 * Útil para recuadros que enmarcan elementos de color variable.
 */
export const getContrastBackgroundColor = (hex: string, opacity = 0.55): string => {
  const { l } = hexToHsl(hex);
  return l > 55 ? `rgba(0,0,0,${opacity})` : `rgba(255,255,255,${opacity})`;
};
...
export const adjustColor = (color: Color, adjustment: { h?: number; s?: number; l?: number }): Color => {
  let { h, s, l } = color.hsl;
  if (adjustment.h !== undefined) h = (h + adjustment.h + 360) % 360;
